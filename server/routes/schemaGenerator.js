...
function extractMetadata(html) {
  ...
  if (jsonLdMatches) meta.existingSchemas = jsonLdMatches.length;
    ...
    if (Array.isArray(value)) {
      if (Array.isArray(merged[key])) {
        ...
        for (const item of value) {
          ...
          if (!existingSet.has(itemKey)) merged[key].push(item);
        }
      } else {
        merged[key] = value;
      }
...
function buildSchemaPrompt(detection, allExtractedData, url) {
  const baseUrl = resolveBaseUrl(url, allExtractedData);
  ...
  const schemaInstructions = detection.schemaTypes
    .map((type) =>
      getSchemaTypeInstructions(type, baseUrl || "https://www.yoursite.com"),
    )
    .join("\n\n");
  const extractedDataStr = JSON.stringify(allExtractedData, null, 2);
...
- Language: ${allExtractedData?._metadata?.language || "en"}
...
7. Include "inLanguage": "${allExtractedData?._metadata?.language || "en"}" on every content entity
8. Include "keywords" from the extracted content keywords, NOT from tool/app metadata

CRITICAL QUALITY RULES:
- UNIQUE DESCRIPTIONS: Each entity MUST have its OWN unique description written from the page content.
...
- BREADCRUMBLIST: Each ListItem MUST have a DIFFERENT url.
...
- If a specific value wasn't extracted, omit that property
...
function checkSchemaCompleteness(schema, requiredTypes) {
  ...
  for (const type of requiredTypes) {
    ...
    if (!schema.includes(`"${type}"`)) missing.push(type);
  }
...
function cleanSchemaResponse(response) {
  ...
  if (scriptStart > 0) schema = schema.substring(scriptStart);
...
const handleGeneration = async (req, res) => {
  ...
  try {
    ...
    console.log(
      `[SchemaGen] Original: ${originalLen} | Cleaned: ${cleanedLen} | Savings: ${savings}%`,
    );
    sendEvent("step", {
      step: "minify",
      status: "done",
      message: `Cleaned: ${originalLen.toLocaleString()} → ${cleanedLen.toLocaleString()} chars (${savings}% reduced)`,
    });
...
Respond in this EXACT JSON format only:
{
  ...
  "aiSeoFocus": "..."
    ...
    console.log(`[SchemaGen] Detection:`, JSON.stringify(detection, null, 2));

    if (!detection || !detection.schemaTypes) {
      sendEvent("error", {
        message:
          "Failed to detect page type. The content might be too short or unclear.",
      });
      res.end();
      return;
    }
...
5. Use ONLY real extracted data. If a specific value like phone/email wasn't found, just omit that one property.
...
REMEMBER: A comprehensive schema with 50+ properties filled from real data is MUCH better than a sparse schema with 5 properties.`,
    ...
    console.log(`[SchemaGen] Schema length: ${finalSchema.length}`);

    // STEP 5b: VALIDATE & REPAIR
    ...
    if (isIncomplete.length > 0) {
      console.log(
        `[SchemaGen] Schema missing types: ${isIncomplete.join(", ")}. Running repair...`,
      );
      sendEvent("progress", {
...
Output the COMPLETE updated <script type="application/ld+json"> block with ALL entities (existing + new).`;
      ...
      if (repairedSchema.length > finalSchema.length) {
        ...
        console.log(
          `[SchemaGen] Schema repaired. New length: ${finalSchema.length}`,
        );
      }
    }
    ...
    console.error("SchemaGen pipeline error:", error);
    sendEvent("error", {
