...
const QUALITIES = [
  ...
  { name: '4k', width: 3840, height: 2160, bitrate: '14000k', audioBitrate: '256k' },
];

export async function processVideoToHLS(videoBuffer, videoId, bucket, originalFilename) {
  const tempDir = path.join(os.tmpdir(), `hls-${videoId}`);
  const inputPath = path.join(tempDir, 'input.mp4');
  const hlsBase = `hls/${videoId}`;
  ...
  try {
    ...
    await uploadToS3(bucket, masterKey, Buffer.from(masterPlaylist), 'application/vnd.apple.mpegurl');
