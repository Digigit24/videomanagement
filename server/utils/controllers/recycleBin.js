...
export async function clearBin(req, res) {
  ...
  try {
    ...
    // Log the real error (including any SQL detail) server-side only.
    console.error("[RecycleBin] Clear failed:", error);
    res.status(500).json({ error: "Failed to clear the recycle bin" });
  }
}
